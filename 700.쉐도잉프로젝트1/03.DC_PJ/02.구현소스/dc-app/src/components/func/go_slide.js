// 배너 컴포넌트 슬라이드 기능 함수 - goslide.js
// 제이쿼리 + 제이쿼리UI
import $ from "jquery";
import "jquery-ui-dist/jquery-ui";

// 생성자 함수로 만들어서 사용하기!
// -> 컴포넌트별로 변수와함수가별도로 메모리구역에 설정됩!
// -> 변수, 함수의 지역화가 가능해짐!-> 같은이름의 변수, 함수로
// 서로 상관되어 엉킬일 없음.

// [ 생성자함수의 필수요건! ]
// 1. 생성자함수는 대문자로 시작하는 이름을 가짐
// 2. 생성자함수는 호출시 new키워드로 생성함
// 3. 생성자함수의 변수, 함수는 this키워드로 노출함
...
function SlideFn() {
   ...
   // this.변수 = 화살표함수 -> 함수인변수를 this키워드로 등록하면 
   // 생성자 함수를 담은 변수에서 하위변수로 노출된다. 그래서 사용가능
this.goSlide = (e) => {
      // 1. 이벤트가 발생한 요소
      const tg = e.target;
      // console.log(tg);
