...
export default function Board() {
   ...
   // -> 특정 리스트 글 제목 클릭시 데이터 저장 (ref는 리랜더링없이 저장댐)
            ...
            <td>
               <a
                  ...
                  onClick={(e) => {
                     ...
                     // *****************************************
                     // 이구역에 set이 먼저있다고 그아래코드 없이 리랜더링 되지않고
                     // 이 코드블럭을 다 실행 함 (selRecord.current = v; 까지 실행후 리랜더링)
                     e.preventDefault();
                     // 읽기모드로 변경!
                     setMode("R");
                     // 해당 데이터 저장하기
  ...
  ******************************************/
   ...
   const submitFn = () => {
      ...
      if (mode == "W") {
         ...
         // 시간까지 나오므로 앞의 10자리만 가져감!
         // 문자열.substr(0,10)  <-- 0인덱스포함해서 10개 
