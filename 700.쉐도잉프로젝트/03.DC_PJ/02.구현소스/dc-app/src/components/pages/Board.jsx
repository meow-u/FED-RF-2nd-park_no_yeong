...
export default function Board() {
  ...
  **********************************************/
   const bindList = () => {
      ...
      // 2. 정렬 적용하기 : 내림차순
      // sort 값이 1이면 desc(현재상태유지 )
      // sort 값이 -1이면 asc (부호반대변경 )
      // 정렬항목은 sortCta 값에 따름 ('idx'/'tit')
      const chgVal = (x) =>
         sortCta == "idx"
            ? //  idx는 숫자형으로 정렬
              Number(x[sortCta])
      ...
      // -> ListMode컴포넌트가 업데이트 되는 동안에
      // 리스트 관련 상태변수를 업데이트 하면
      // 업데이트 불가 에러 메시지가 발생한다!
      // 따라서 이런 코드는 다른 방식으로 변경해야함!

      return (
         // 전체 데이터 개수가 0 초과일 경우 출력
         // 0 초과 ? map돌기 코드: 없음코드(3항연산자)
      ...
      <>
         <div className="selbx">
            ...
            <input
               ...
               maxLength="50"
               onKeyUp={(e) => {
                // e.keyCode는 번호로 13이 벤터
                // e.key는 문자로 Enter가 엔터
            ...
            <tfoot>
               <tr>
                  <td colSpan="5" className="paging">
                     {totalCount.current > 0 && ( //데이터 갯수가 0 이상일때만 출력
                        <PagingList
                           totalCount={totalCount}
                           unitSize={unitSize}
                           pageNum={pageNum}
