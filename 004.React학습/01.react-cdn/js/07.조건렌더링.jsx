...
function LostDeveloper() {
   return <h1>리액트가 뭐지?</h1>;
} /////// LostDeveloper 컴포넌트 ///////

// 3번 컴포넌트 ////////////////////////
function MakeImage({ isrc, ialt, itit, icss }) {
   // isrc-파일경로, ialt-설명, itit-툴팁, icss-스타일설정
...
/****************************************************** 
    ...
    and 조건, or 조건, not 조건식이 모두 사용가능 함!
...
function MakeList({ foodName, movieInfo }) {
   ...
   return (
      <li>
         {
            // 음식데이터가 들어온경우 (undefined)가 아니면true이므로
            // && 뒤 코드 출력
            // 표현식내부라서 변수는 중괄호없이 문자열은 문자열표시
            // 변수와 문자열을합칠때는 사이사이 +
...
function WishList({ wList }) {
   ...
   return (
      <React.Fragment>
         ...
         {/* 배열값이 0인 경우 다른것 출력하기 */}
         {wList.length === 0 && (
            <h2> 아직 개발자가 음식리스트가 업데이트 되지 않았습니다.</h2>
         )}

         {wList.length > 0 && <h2>무야호~~~!!!!!!</h2>}
      </React.Fragment>
   );
} ///////////// WishList 컴포넌트 ////////////////////
////////////////////////////////////////////////////
...
// 3-1. 개발자 선호 영화 리스트 출력 컴포넌트
// 제목컴포넌트, 리스트 컴포넌트 모두 재활용
function MovieWishList({ wList }) {
   return (
      <React.Fragment>
         {/* // 영화 위시리스트 타이틀 출력하기  */}
         <SetTitle title={"영화"}></SetTitle>
         {/* 영화리스트출력 */}
         {wList.length > 0 && (
            <div>
               <h2>
                  개발자가 좋아하는 영화는 최근 {wList.length}년간 아래와
                  같습니다.
               </h2>
               <ul>
                  {wList.map((v) => (
         ...
         {wList.length == 0 && (
            <div>
               <h2>아직 개발자 영화리스트가 업데이트되지 않았습니다.</h2>
            </div>
         )}
      </React.Fragment>
   );
...
function ShowLikePainter({ isChange }) {
   // isChange - 기존작가를 변경하는 여부 전달 변수
   // true / false -> true일시 작가 바꿈 !!
   // 비?집:놀이동산 으로 그림출력파트를 변경함!


   
// [후크 상태관리 변수 만들기] (result는 맘대로 , set그변수)
const [result,setResult] = React.useState(isChange);
   ...
   return (
      <React.Fragment>
         ...
         {/* 3. 작품출력 */}
         {
            // 삼항연산자로 isChange값에따라 출력변경하기
            // isChange가 아닌 상태관리변수를 사용한다!
            result ? (
               <MakePainting painter="피카소" wname="우는여인" />
            ) : (
               <MakePainting painter="모네" wname="양산을쓴여인" />
            )
         }
      </React.Fragment>
   );
