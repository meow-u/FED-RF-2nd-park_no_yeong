...
function MakeImage({ isrc, ialt, itit, icss }) {
   ...
   return (
   <img src={isrc} alt={ialt} title={itit} style={icss}/>);
} ///////// MakeImage 컴포넌트 ////////
   ...
   return(
      <React.Fragment>
         ...
         <div>
            <h2>
               개발자가 좋아하는 영화는
               최근 {wList.length}년간 아래와 같습니다.
            </h2>
            <ul>
               { wList.map(v=> <MakeList movieInfo={v}/>) }
            </ul>
            {/* 영화포스터 이미지 영화순서대로 뿌리기  */}
            { wList.map(v=> 
            <MakeImage  isrc={v.poster} ialt={v.mtit} 
            icss={
               {width:'100px'}
            } />)}
         </div>
