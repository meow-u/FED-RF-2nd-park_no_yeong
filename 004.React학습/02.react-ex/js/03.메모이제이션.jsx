...
function App() {
   ...
   // 국적표시 객체 score가 바뀌면 app이 리랜더링되면서 코드를 다시 읽을떄
   //nara의 데이터객체 메모리 저장 주소가 바뀌어서
   //값이 똑같아도 nara가 바뀐것으로 인지한다

   // -> 매번 컴포넌트가 리랜더링 될때마다 할당되는 객체의 주소값이 변경된다.
   //    const nara = {
   //       country: isKor ? "한국" : "일본",
   //    };

   // 처음 할당된 객체의 주소값을 메모하여 재사용하면
   // 다른 상태변수가 변경될 때 같은  주소값을 유지하여
   // nara변수가 변경되지 않도록 한다!
   // 사용법: 변수 = React.useMemo(()=>{return객체},[의존성]);
   ///////////////////////////////////////////////////////////////////
   const nara = React.useMemo(() => {
      return {
         country: isKor ? "한국" : "일본",
      };
   }, [isKor]); // -> 의존성이바뀌지않는한 주소도 같은값을 유지
   ///////////////////////////////////////////////////////////////////
   ...
   /******************************************************** 
    ...
    (nara의 메모리주소를 동일하게 재사용해서 nara가 바뀌지않음)
  ********************************************************/
   React.useEffect(() => {
      console.log("nara가 변경됨!");
      // 축구선수 이미지 중 해당 나라 이미지가 위로 올라옴
      //eq는 시퀀스(순번)기존값에 50px을합쳐줌
      $("img")
...
// [제목을 만들어주는 서브컴포넌트]////////////
// -> 초기셋팅 후 변경내용이 없는 서브컴포넌트가 
// 리랜더링 되지 않도록 하는 방법은? ->React.memo( ()=>{} )
// 주의사항: 컴포넌트가 반드시 ""할당형"" 으로 만들어져야 함 (변수 화살표함수)

// 이떄 txt가 함수가 들어오면 메모이제이션이 안된다 (배열/객체/함수는 주소값 새로할당)
const ShowTit =React.memo(({txt})=>{
   console.log("App컴포넌트 내부의 Show Tit 컴포넌트 랜더링");
