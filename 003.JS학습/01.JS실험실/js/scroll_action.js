...
/**************************************************** 
  ...
  3. 스크롤 등장 대상요소의 보이는 화면에서의 top값
    ...
    ((메서드명 조어 분석))
     ...
     Client 보이는화면 
     Rect 사각형 
     ->>> BoundingClientRect -> 보이는 화면 
     사각형 경계선으로부터의 거리를 리턴해주는 메서드
     -> 상단으로 부터의 거리는 top속성 
     -> 왼쪽으로 부터의 거리는 left
...
//    console.log("대상:", stage);
 ...
 >${x}</span>`;
   ...
   /* 배열상자만 만들어두고 
   scAct idx순번만큼 배열을 만들어서 posTop값 넣음 */
   ...
   // 화면 맨위에 걸린 상태와 같음 
   //(이때의  window.scrollY 값이 offsetTop과 동일함)
   ...
   console.log('포스터위치(ele.offsetTop):', posTop, gap);
