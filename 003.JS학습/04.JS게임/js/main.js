...
// -> assert 키워드 지원중단됨 (사용은가능) 대신 with 키워드권장
// -> 같이써 타입제이슨!
...
// console.log('대상:',t1,r1,btns,level,msg,r1pos,t1pos);
...
function goGame(){
    ...
    if (btxt === '토끼출발'){
      ...
      // (1) 거북 멈춤 상태값이 true이면  이 gogame함수나가!(return)
      if(t1Stop) return;
      ...
      // (3) 거북버튼 클릭 후 포커스로 인해  엔터버튼을
      // 사용할 수 있으므로 이를 막기위해 포커스 해제 
      // 즉, blur()메서드로 처리함
      this.blur();
