...
*************************************************************
      ...
      (6) 코드예 :
          ...
          return true 하여 할당된 변수에 저장하고 문장이 바로 끝난다!
           ...
           -1을 반환하기로 한 것은, 존재하지 않는 것을 의미하는 특별한 값으로 약속한 것입니다. 어떤 유효한 인덱스도 -1이 될 수 없기 때문입니다.
          이렇게 -1을 반환함으로써, 프로그래머가 쉽게 "찾는 문자열이 없다"는 것을 알아챌 수 있습니다. 그래서 많은 언어와 라이브러리에서 이런 관행을 따르고 있습니다.
...
// (2) map() 메서드 없이 foreach()로 배열값을 이미지태그로
// 변환하여 코드 만들기 함수
const returnTag = (x) => {
  // x 는 배열 전달변수
  // 1.태그 저장용 변수
...
***********************************************/
//  map 메서드를 사용하여 배열값을 태그로 감싸서 출력하기 예시
// console.log("원본 배열:", arrNumber2);

// console.log(
//   "원본 배열로 태그작성:맵돌리면 배열로 나옴",
//   arrNumber2.map((v) => `<숫자>${v}</숫자>`)
// );

// console.log(
//   "원본 배열로 태그작성한 배열을 문자열로 변경하기: 조인하면 무조건 문자형이됨",
//   arrNumber2.map((v) => `<숫자>${v}</숫자>`).join("")
// );

// // 원본 배열로 데이터작성하여 새배열에 할당하기
// const newArray = arrNumber2.map((v, i) => `👓회원번호${i + 1}:${v}포인트`);
// console.log("원본 배열로 데이터작성:", newArray);

// console.log('새배열:',newArray,"\n원본 배열:그대로임 ", arrNumber2);

/////////////////////////////////////////////////////

// (3) map() 메서드를 이용해 배열 숫자 데이터 만큼 이미지로 변환하여 화면출력
// arrObj는 전달된 배열
...
function sortingFn(evt, cta, arrData, exBox) {
  ...
  // 위 방법의 작동방식은 객체를 문자열로 변환후 다시 객체로
  // 변환하는 방식으로 객체의 주소값을 끊어버린다!
  //위에서 JSON.Stringify()는 객체를 문자열로 변환하고
  // 이때의 변환된 문자열을 적자면
  // "[{},{},{}]" 이런식으로 배열안에 객체가 문자열로 변환된다!
  // JSON.parse()는 문자열을 객체로 변환한다!
  // 이렇게 하면 객체의 주소값이 끊어지므로
  ...
  if (selVal == 1) {
    ...
    // 여기서 a는 앞값, b는 뒷값이며 이 list1의 객체데이터 배열이다! a는 앞값, b는 뒷값이다!
    // 여기서 앞값의 데이터는 a[cta]로 읽어오고 뒷값의 데이터는 b[cta]로 읽어온다! 읽어진 데이터를 적자면 a는 list1[0].idx인 8이고 b는 list1[1].idx인 4이다! 이런식으로 sort()메서드는 배열값을 비교하여 순서를 바꾼다!
    // idx만 비교하여 정렬하므로 둘중 idx값이 작은 4가 속한 객체가 앞으로 오게된다! 이렇게 전체 배열을 순회하여 정렬하게 된다!
...
mFn.addEvt(btnTotal, "click", () => {
  ...
  // 3. 검색결과 초기화
  //검색완료후 전체버튼누르면 다시 전체에서 정렬할수있도록
  searchResult = null;

  //4. 전체버튼 선택시 정렬선택값을 초기화 value "0"으로 초기화
  sel4.value = "0";
...
function searchingFn() {
  ...
  // 7. 검색후엔 항상 정렬선택값을 초기화 value "0"으로 초기화
  sel4.value = "0";
...
// searchResult값에 할당되어 true이면 그값을 보내고 
// 아니면 원본 list2를 보낸다.
// (4) 정렬기준 대상 선택 변경시
// -> 정렬종류 대상 초기화하기("정렬선택"으로 변경!)
