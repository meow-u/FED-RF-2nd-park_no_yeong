...
// 2-2. 변경 대상 : .showit ( 파란떙떙이 배열정보출력) / .cont ( 하단(지금안보임) 과일출력박스)
const showit = mFn.qs(".showit");
const cont = mFn.qs(".cont");

// 2-3. 전체과일 콤보박스(누르면열리는네모) : #sel
const sel = mFn.qs("#sel");

// 2-4. 선택과일 콤보박스 (누르면열리는네모)  (anum=array number) : #anum
...
// 3. 초기화 작업 : 처음배열 출력 / 콤보박스바인딩(묶는단뜻)

// 3-1. 처음배열출력
// fruit 배열 변경시 다시 출력해야 하므로 함수로 만들기
const showArray = () => {
   showit.innerText = fruit.join("♥");
   // ★★배열.join(구분자) : 배열값 사이에 구분자를 넣은 '문자열값 '반환
}; ////// showArray 함수 ////////
// 처음배열 출력함수 최초호출 아랫쪽에서!!
showArray();
...
// console.log('변환후 키 배열:',arrFruits);
// console.log('변환후 값 배열:',Object.values(frObj));

// 기존배열값을 태그로 변환하여 다시 배열로 할당하기

// -> ★★★★ 배열.map( (v,i,arr)=>리턴값  ) 메서드  // 이 리턴값자리에 출력될수 있는것만 넣기
// -> ★★★★ 기본 forEach() 메서드와 전달값이 같음!!
// -> ★★★★ v배열값, i 순번, arr 전체 배열

// ->>> 기본배열값을 순회하면 변환된 값을 다시 넣어줌! //꺼냇다가 바꿔서 원래 위치에다시 집어넣어줌!!!
// arrFruits.map(v=> `<option>${v}</option>`);  <---이렇게하면 변경은했지만 원본이 바뀐건아님
// map(v=> 이뒤에 {} 중괄호를 쓰게되면 리턴을 일일히 써야 리턴값이 출력되니 그냥
// 중괄호없이 `` 백틱안에 내용을 적는다.(그럼 리턴생략)
...
// 화살표 함수에서 중괄호를 사용하면 리턴 할 경우 return 키워드를 꼭 써야한다.
// 따라서 중괄호 생략시 리턴키워드 자동 생략

// 배열값을 문자화 하여 콤보박스에 태그넣기
// 그냥배열을 할당하면 콤마가 들어감!
// 그러므로 join메서드로 콤마를 없애서 넣는다!
...
function showFruit() {
  ...
  // -> 결과: 선택순번 앞쪽에 배열값이 삽입됨 
    ...
    // 대상 :fruit 
    fruit.splice(aNum.value,0,sel.value);  //강아지,고양이 뒤 붙이면나옴
