...
/*************************************************************** 
    ...
    (1) 장점: 간단한 프론트엔드 데이터를 DB없이 테스트해보는데 탁월함
        쿠키와 같이 사용자 동의를 필요로 하는 로컬 기록 수단의 단점은
        이것을 거부하면 쓸수 없는데, js스토리지는 이런 제약이 없다!
        보안상 안정이 보장되므로 사용가능 함!
...
function localsFn() {
   ...
   else if (btxt == "처리::로컬에minfo없음만들고 바인딩해서게시판뿌리기") {
      // 배열/객체 만들기
...
function bindData() {
   ...
   mFn.qsa(".del-link a").forEach((ele) => {
      ele.onclick = (e) => {
        ...
        if(!confirm('정말 지우시게요?'))return; 
        // 취소를 누르면 !false로 true가 되어 함수를 빠져나가서 아래가실행안됨
        // confirm()앞에 not연산자를 써서 ->> false일떄 리턴(나간다)한다.

         // 3. x의 지울순번속성(data-idx)읽어오기
...
mFn.qs("#sbtn").onclick = () => {
   ...
   // 2. 공백제거한 입력값이 비었으면 돌려보내기!
   // trim() - 앞뒤공백 제거 메서드
   if (
      mFn.qs("#tit").value.trim() == "" ||
...
mFn.addEvt(selBox,'change',(e)=>{
    ...
    // 데이터 선택하기 -> 변수 = 배열.find(v->{if(조건){return true}})

    let selRec = localData.find(v=>{
        console.log(v.idx);//찾으면 돌다가 딱 끝남
...
mFn.qs("#mobtn").onclick = () => {
   ...
   // 3. 공백제거한 입력값이 비었으면 돌려보내기!
   // trim() - 앞뒤공백 제거 메서드
   if (
    mFn.qs("#tit2").value.trim() == "" ||
