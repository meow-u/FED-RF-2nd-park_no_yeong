...
function loadFn() {
   ...
   function goSlide(evt,sts=true){ 
    ...
    // 전달변수를 하나만적으면 이벤트객체가 담기니까 일부로 2개적음
    // sts- 버튼클릭인지 자동호출인지 구분하는 전달변수
    // -> true면 버튼클릭 , false면 자동호출로 구분
    // : 버튼클릭시에 아무것도 안보내므로 기본값
    // true가 할당되어 적용됨!!!

    // 언디파인드는 > 할당된적없는것!
    // null 빈값은 >할당되었으나 값이 비어있는것!!

    // -> 만약 전달값이 없으면 기본값으로 셋팅함!
    // -> ES6문법에서 전달변수 초기값주기 문법생김!
    ...
    if(sts){ 
      ...
      // <-- false면 let  isRbtn 에  true를 할당
      // sts가 true냐? 맞으면 버튼을 클릭한 것이므로 
      // this 키워드에 의한 클래스 .ab2존재여부를 물어라
      // false냐? 맞으면 무조건 true 값을 할당해라!
      // 왜? 자동 넘김은 오른쪽버튼 클릭한 방향으로 가야하니까!
