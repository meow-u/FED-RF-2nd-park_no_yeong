...
/* 슬라이드 덩어리 하나or 여러개에 따라 512번째 줄 위아래수정해서쓰기  */
...
function slideFn(selEl) {
   ...
   ********************************************/
   ...
   const dMove = (e) => {
      ...
      /* ************************************ */
      // 드래그시 이동버튼+블릿 포인트 이벤트 없앰설정하기
      // 상위 selEl에 클래스 .no 주면 된다!
      if (dragSts) {
         selEl.classList.add("no");
      } else {
         selEl.classList.remove("no");
      }
      /* ************************************ */
   ...
   const moveDdragSlide = () => {
      ...
      if (currentLeft < valFirst) {
         ...
         // valFirst valSecond의 사이범위
         console.log("제자리!");
         slide.style.left = "-220%";
         slide.style.transition = ".3s ease-in-out";
      } ////else

      // 드래그 시 더해지는 마지막위치값 lastX를
      // -220% left PX값으로 초기화해준다(숫자만!)
      lastX = originalValue;
      // -> 이것을 해야 오작동이 없다!!
