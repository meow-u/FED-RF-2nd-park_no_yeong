...
// 박스 생성 후 호출 (큐로보내면 스택실행 후 호출, 아래 라이브하게 적힌 코드들은 스택)
// 셋인터발, 셋타임아웃으로 가둔 코드들은 큐 (스택끝난뒤 실행)
setTimeout(showSubBox,0);
// 시간을 0으로 써도, 심지어 시간을 안써도 setTimeout()으로 함수를 호출하면
// 스택에 실행 후 큐에서 가지고 있다가 스택코드실행이 모두 끝난 후 호출하여 실행한다 
...
// !(NOT)연산자로 반대로 뒤집으면 false 일때 처리함! 
if(!pos) return

// 2-2. 해당 박스 아이디의 위치값 알아내기 
// offset().top -> top 위치값정보 (제이쿼리)  
pos = $(pos).offset().top 
console.log('위치값:', pos)
...
800, //시간
'easeOutBack', // 이징(http://easings.net)
// 콜백함수(애니후 호출되는 함수)
()=>{
// 이동후 부드러운 스크롤 위치값 업데이트 필수 
// 이것 안하면 제이쿼리로 스크롤값을 기존 값이 인지하지못해서 이후에 스크롤할때 위치가 튐  
