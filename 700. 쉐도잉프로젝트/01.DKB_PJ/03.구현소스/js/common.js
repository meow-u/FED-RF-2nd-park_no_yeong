...
function bindCombo(){
  ...
  // 데이터대상: comboData.brand

  // 대상요소 내부데이터 초기화
  // 배열데이터.map(),join('')
  brandBox.innerHTML = 
  `<option value="init">브랜드바로가기</option>`+
  ...
  // 데이터분석: 객체로된 데이터 이므로 map()을 쓰려면 
  // 객체의 키(key)를 배열로 추출하여 사용한다!
  // Object.keys(객체) -> 키 배열!
  // 객체의 값을 사용할 경우는 원본객체[키]

  // 데이터대상:  comboData.corp
...
function openWindow(){
   ...
   // 0. option값이 'init'일 경우 돌아가!
  //  if(this.value == "init") return;   --> 아래 if(url)문으로 처리함
  
   // 1. 이동할 주소 : comboData.brandLink 또는 comboData.corpLink 객체 선택
   // 객체 이름조합을 (아이디명+"Link")
   // 그 하위의 option값을 url 값으로 가져옴!!!
   
   let url = comboData[this.id+'Link'][this.value];
   console.log('브랜드어디?',url);
   
   // -> 만약 데이터가 없으면 url변수의 값은 셋팅되지 못하여
   // undefined 처리된다! 이것을 if문으로 처리하여
   // 아래 새창띄우기 코드를 감싸준다!
   // url 값이 셋팅되지 않으면 새창열기 코드는 실행되지 않음
   // 따라서 위의 "init" 코드로 별도의 처리가 불필요함

   if(url){//값이 있을때만 열기
    window.open(url)}else alert("선택을 변경해주세요!");
     // 1. 선택 option 값의 주소로 이동하기
